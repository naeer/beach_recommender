...
def insert_data_to_table(dbname, tablename, data):
    strdata=tuple(data.split(","))
    print(strdata)
    script_sql_add = f"""INSERT INTO "main"."{tablename}" VALUES {strdata};"""
...
def execute_query_and_fetch_results(dbname, query):
    try:
        sqliteConnection = sqlite3.connect(str(dbname))
        cursor = sqliteConnection.cursor()
        print("Successfully Connected to SQLite")
